...
public class TPCHHCatalogQ6 extends Configured implements Tool {
			...
			if (isLogInfoEnabled) {
				...
				if (numRows == nextCntr) {
					long used_memory = memoryMXBean.getHeapMemoryUsage()
							.getUsed();
					l4j.info("TPCHHCatalogQ6.Map: processing " + numRows
			...
			while (iter.hasNext()) {
				...
				if (isLogInfoEnabled) {
					...
					if (numRows == nextCntr) {
						long used_memory = memoryMXBean.getHeapMemoryUsage()
								.getUsed();
						l4j.info("TPCHHCatalogQ6.Combine: processing " + numRows
								+ " rows: used memory = " + used_memory);
						nextCntr = getNextCntr(numRows);
					}
				}
			...
			while (iter.hasNext()) {
				...
				if (isLogInfoEnabled) {
					...
					if (numRows == nextCntr) {
						long used_memory = memoryMXBean.getHeapMemoryUsage()
								.getUsed();
						l4j.info("TPCHHCatalogQ6.Reduce: processing " + numRows
								+ " rows: used memory = " + used_memory);
						nextCntr = getNextCntr(numRows);
					}
				}
	...
	public static void main(String[] args) throws Exception {
		int exitCode = ToolRunner.run(new TPCHHCatalogQ6(), args);
